...
export const useTaskStore = defineStore('tasks', () => {
  ...
  const toggleTask = async (dayIndex: string, taskId: string) => {
    ...
    const taskToToggle = currentWeek.value.tasks[dayIndex].find(task => {
      // 通过类别和名称匹配任务
      const templateTask = currentWeek.value?.tasks['1'].find(t => t.id === taskId);
      return task.category === templateTask?.category && task.name === templateTask?.name;
    });
  ...
  const completionPercentage = computed(() => {
    ...
    return Math.round((currentWeek.value.earnedPoints / currentWeek.value.totalPoints) * 100)
  })
